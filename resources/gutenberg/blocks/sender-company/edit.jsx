...
const edit = ( props ) => {
	...
	useEffect( () => {
		if ( meta && meta.pc_sender_company ) {
			setAttributes( { content: meta.pc_sender_company } );
		}
	...
	return (
		<>
			...
			<BlockDiv attributes={ attributes }>
				<RichText
					tagName="p"
					className={ classnames( 'pc-sender-company' ) }
					value={ meta?.pc_sender_company || '' }
...
const applyWithDispatch = withDispatch( ( dispatch, { setAttributes } ) => {
	...
	return {
		updatePageTitle: ( postId, newTitle ) => {
			postType.setMetaData( postId, 'pc_sender_company', newTitle );
		},
	};
} );
