...
const edit = ( props ) => {
	...
	useEffect( () => {
		if ( meta && meta.pc_client_email ) {
			setAttributes( { content: meta.pc_client_email } );
		}
	...
	return (
		<>
			...
			<BlockDiv attributes={ attributes }>
				<RichText
					tagName="p"
					className={ classnames( 'pc-client-email' ) }
					value={ meta?.pc_client_email || '' }
...
const applyWithDispatch = withDispatch( ( dispatch, { setAttributes } ) => {
	...
	return {
		updatePageTitle: ( postId, newTitle ) => {
			postType.setMetaData( postId, 'pc_client_email', newTitle );
		},
	};
} );
