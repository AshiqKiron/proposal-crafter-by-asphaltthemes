...
const edit = (props) => {
	...
	useEffect(() => {
		if (meta && meta.pc_client_name) {
			setAttributes({ content: meta.pc_client_name });
		}
	}, [meta?.pc_client_name]);
	...
	return (
		<>
			...
			<BlockDiv attributes={attributes}>
				<RichText
					tagName="p"
					className={classnames('pc-client-name')}
					value={meta?.pc_client_name || ''}
...
const applyWithDispatch = withDispatch((dispatch, { setAttributes }) => {
	...
	return {
		updatePageTitle: (postId, newTitle) => {
			postType.setMetaData(postId, 'pc_client_name', newTitle);
			editPost({ meta: { pc_client_name: newTitle } });
