...
const edit = ( props ) => {
	...
	useEffect( () => {
		if ( meta && meta.pc_sender_name ) {
			setAttributes( { content: meta.pc_sender_name } );
		}
	}, [ meta?.pc_sender_name ] );
	...
	return (
		<>
			...
			<BlockDiv attributes={ attributes }>
				<RichText
					tagName="p"
					className={ classnames( 'pc-sender-name' ) }
					value={ meta?.pc_sender_name || '' }
...
const applyWithDispatch = withDispatch( ( dispatch, { setAttributes } ) => {
	...
	return {
		updatePageTitle: ( postId, newTitle ) => {
			postType.setMetaData( postId, 'pc_sender_name', newTitle );
		},
	};
} );
