...
const edit = ( props ) => {
	...
	useEffect( () => {
		if ( meta && meta.pc_sender_email ) {
			setAttributes( { content: meta.pc_sender_email } );
		}
	...
	return (
		<>
			...
			<BlockDiv attributes={ attributes }>
				<RichText
					tagName="p"
					className={ classnames( 'pc-sender-email' ) }
					value={ meta?.pc_sender_email || '' }
...
const applyWithDispatch = withDispatch( ( dispatch, { setAttributes } ) => {
	...
	return {
		updatePageTitle: ( postId, newTitle ) => {
			postType.setMetaData( postId, 'pc_sender_email', newTitle );
		},
	};
} );
